...
class ReportService
{
    ...
    public function getPeriodSummary(int $tenantId, string $startDate, string $endDate): array
    {
        ...
        // Payment breakdown - DYNAMIC based on actual payment methods
        $paymentMethods = $orders->whereNotNull('payment_method')
        ...
        foreach ($paymentMethods as $method) {
            $methodOrders = $orders->where('payment_method', $method);
            $methodAmount = $methodOrders->sum('total_amount');
            
            $paymentBreakdown[] = [
                'method' => $method,
                'amount' => (float) $methodAmount,
                'count' => $methodOrders->count(),
                'percentage' => $netSales > 0 ? round(($methodAmount / $netSales) * 100, 2) : 0,
            ];
        }
        
        // Sort by amount descending
        usort($paymentBreakdown, function($a, $b) {
        ...
        return [
            ...
            // Enhanced sections for period report
     ...
     * Note: Orders table doesn't track staff, returning empty for now
     */
    protected function getPeriodStaffPerformance(int $tenantId, Carbon $start, Carbon $end): array
    {
        // TODO: Add user_id or staff_id column to orders table to track staff performance
